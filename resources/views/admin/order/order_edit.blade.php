    ...
    <div class="x-body">
        <form class="layui-form">
            ...
            <div class="layui-form-item layui-form-text">
                ...
                <div class="layui-input-block">
                    <input type="radio" title="已提交" name="status"
                           value="1" {{$order_data->status==1?'checked':''}} {{$order_look==1 ? 'disabled' :''}}>
                    <input type="radio" title="待发货" name="status"
                           value="2" {{$order_data->status==2?'checked':''}} {{$order_look==1 ? 'disabled' :''}}>
                    <input type="radio" title="已发货" name="status"
                           value="3" {{$order_data->status==3?'checked':''}} {{$order_look==1 ? 'disabled' :''}}>
                    <input type="radio" title="待收货" name="status"
                           value="4" {{$order_data->status==4?'checked':''}} {{$order_look==1 ? 'disabled' :''}}>
                    <input type="radio" title="已完成" name="status"
                           value="5" {{$order_data->status==5?'checked':''}} {{$order_look==1 ? 'disabled' :''}}>
                    <input type="radio" title="已作废" name="status"
                           value="0" {{$order_data->status==0?'checked':''}} {{$order_look==1 ? 'disabled' :''}}>
                    ...
                    success: function (res) {
                        if (res.code == 0) {
                            layer.alert("修改成功", {icon: 6}, function () {
                                ...
                                parent.layui.table.reload('order_list');
                            });
                            return false;
                        } else {
