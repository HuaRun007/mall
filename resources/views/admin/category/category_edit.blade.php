    ...
    <div class="x-body">
        <form class="layui-form" >
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <select name="parent_id" lay-verify="required">
                        ...
                        <option value="0" {{$category_data->parend_id==0? 'selected' : ''}}>顶级分类</option>
                        @forelse($categorys as $item)
                            ...
                            @if(isset($item['_child']))
                                @foreach($item['_child'] as $item2)
                                    <option value="{{$item2['id']}}" {{$category_data->parent_id==$item2['id'] ? 'selected' : ''}} >&nbsp;&nbsp;┗━━{{$item2['name']}}</option>
                                    @if(isset($item2['_child']))
                                        @foreach($item2['_child'] as $item3)
                                            <option value="{{$item3['id']}}" {{$category_data->parent_id==$item3['id'] ? 'selected' : ''}} >&nbsp;&nbsp;&nbsp;&nbsp;┗━━{{$item3['name']}}</option>
                                        @endforeach
                                    @endif
                                @endforeach
                            @endif
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <input type="text" id="L_name" name="name" lay-verify="required"
                           autocomplete="off" class="layui-input" value="{{$category_data->name}}"   >
                </div>
                <div class="layui-form-mid layui-word-aux">
                    <span class="x-red">*</span>
                </div>
                    ...
                    <div class="layui-upload">
                        ...
                        <div class="layui-upload-list" >
                            <ul id="layui-upload-box2" class="layui-clear">
                                <li><img src="{{$category_data->preview}}" /></li>
                            </ul>
                            <input type="hidden" value="{{$category_data->preview}}" id="preview" name="preview">
                        </div>
                    </div>
                </div>
            ...
            <div class="layui-form-item">
                ...
                <button  type="button" class="layui-btn" lay-filter="category_edit" lay-submit="">
                    修改分类
                </button>
            </div>
        </form>
    ...
    <script>
        layui.use(['form','layer','upload'],function () {
            ...
            form.on('submit(category_edit)',function(data){
                ...
                $.ajax({
                    type: 'post',
                    url : "{{url('/admin/category/update',['category'=> $category_data])}}",
