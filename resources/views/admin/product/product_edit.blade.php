    ...
    <div class="x-body">
        <form class="layui-form" >
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <input type="text"  name="name" lay-verify="required"
                           autocomplete="off" class="layui-input" value="{{$product_data->name}}">
                </div>
                ...
                <div class="layui-input-inline">
                    <select name="category_id" lay-verify="required">
                        ...
                        @forelse($categorys as $item)
                            <option value="{{$item['id']}}" {{$product_data['category_id']==$item['id'] ? 'selected':''}}>{{$item['name']}}</option>
                            @if(isset($item['_child']))
                                @foreach($item['_child'] as $item2)
                                    <option value="{{$item2['id']}}"}} {{$product_data['category_id']==$item2['id'] ? 'selected':''}}>&nbsp;&nbsp;┗━━{{$item2['name']}}</option>
                                    @if(isset($item2['_child']))
                                        @foreach($item2['_child'] as $item3)
                                            <option value="{{$item3['id']}}" {{$product_data['category_id']==$item3['id'] ? 'selected':''}}>&nbsp;&nbsp;&nbsp;&nbsp;┗━━{{$item3['name']}}</option>
                                        @endforeach
                                    @endif
                                @endforeach
                            @endif
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <input type="text"  name="price" required="" lay-verify="number|required"
                           autocomplete="off" class="layui-input" value="{{$product_data->price}}">
                </div>
                <div class="layui-form-mid layui-word-aux">
                    <span class="x-red">*</span>
                </div>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <input type="radio" name="on_sale" value="1" title="在售" {{$product_data->on_sale==1?'checked':''}}>
                    <input type="radio" name="on_sale" value="0" title="下架" {{$product_data->on_sale==0?'checked':''}}>
                </div>
                <div class="layui-form-mid layui-word-aux">
                    <span class="x-red">*</span>
                </div>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <input type="text"  name="rating" required="" lay-verify="number"
                           autocomplete="off" class="layui-input" value="{{$product_data->rating}}">
                </div>
            </div>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <input type="text"  name="sold_count" required="" lay-verify="number"
                           autocomplete="off" class="layui-input" value="{{$product_data->sold_count}}">
                </div>
            </div>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <textarea name="description" id="" class="layui-textarea">{{$product_data->description}}</textarea>
                </div>
            </div>
                    ...
                    <div class="layui-upload">
                        ...
                        <div class="layui-upload-list" >
                            <ul id="layui-upload-box2" class="layui-clear">
                                <li><img src="{{$product_data->preview}}" /></li>
                            </ul>
                            <input type="hidden" value="{{$product_data->preview}}" id="preview" name="preview">
                        </div>
                    </div>
                </div>
                        ...
                        <blockquote class="layui-elem-quote layui-quote-nm" style="margin-top: 10px;">
                            ...
                            <div class="layui-upload-list" >
                                <ul id="layui-upload-box" class="layui-clear">
                                    @forelse($image_data as $item)
                                        <li id="{{$item['id']}}" image_name="{{$item['image_path']}}">
                                            <i class="layui-icon layui-icon-delete"  onclick="UPLOAD_IMG_DEL({{$item['id']}})"></i>
                                            <img src="{{$item['image_path']}}"/>
                                            <input type="hidden"  name="image_src"  value="{{$item['image_path']}}">
                                        </li>
                                    @empty
                                    @endforelse
                                </ul>
    ...
    <script>
        layui.use(['form','layer','upload'],function () {
            ...
            form.on('submit(product_add)',function(data){
                ...
                $.ajax({
                    type:'post',
                    dataType:'json',
                    url:"{{url('/admin/product/update',['product'=>$product_data])}}",
                    data:data.field,
        ...
        */
        function UPLOAD_IMG_DEL(lis) {
            var path =$("#"+lis).attr('image_name');
            layui.layer.confirm('确定删除吗？改图片将彻底删除',function(){
                $.ajax({
                    url:'/deleteImg_database/'+lis,
                    type:'post',
