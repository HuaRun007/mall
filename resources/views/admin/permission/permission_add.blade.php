                        ...
                        @forelse($permission as $item)
                            ...
                            @if(isset($item['_child']))
                                @foreach($item['_child'] as $item2)
                                    <option value="{{$item2['id']}}">&nbsp;&nbsp;┗━━{{$item2['display_name']}}</option>
                                    @if(isset($item2['_child']))
                                        @foreach($item2['_child'] as $item3)
                                            <option value="{{$item3['id']}}">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;┗━━{{$item3['display_name']}}</option>
                                        @endforeach
                                    @endif
                                @endforeach
                            @endif
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <input type="text" id="L_name" name="name" lay-verify="required"
                           autocomplete="off" class="layui-input" value=""  placeholder="格式如：system.manage" >
                </div>
                <div class="layui-form-mid layui-word-aux">
                    <span class="x-red">*</span>
                </div>
